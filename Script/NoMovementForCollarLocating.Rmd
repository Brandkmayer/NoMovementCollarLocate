...
Meta<- read_csv(row.names(Metafiles)[1]) %>% select("collar" = DeviceEUI,EarTag,HerdName) #%>% mutate(HerdName = "Small Herd") 
...
for (i in 1:length(NMC_listed)) {
  ...
  for (j in 1:(nrow(NMC_listed[[i]])-1)) {
    NMC_listed[[i]]$Dist[j+1]<- distm(c(NMC_listed[[1]][1,5][[1]],
                                      NMC_listed[[i]][j,4][[1]]),
...
NoMovementScreen <- NMC_NMcleaned[NMC_NMcleaned$ErrorGPS >32,]
